    ...
    simulation = d3.forceSimulation()
        .force("link", d3.forceLink()
            ...
            .distance(function (d) {
                ...
                // cluster edges
                if (d.isCluster) return length_link_between_cluster_nodes;
...
function generate_hidden_graph(graph) {
    ...
    // create the anchor nodes
    graph["nodes"].push({ id: "f1", isFixed: true, fx: 80, fy: 350 });
    graph["nodes"].push({ id: "f2", isFixed: true, fx: 900, fy: 350 });
...
function draw_hidden_graph(graph) {
    ...
    var link = svg.append("g")
        ...
        .style("stroke-opacity", function (d) {
            if (d.isHidden || d.isFixed || d.isCluster) return 0;
            else return 1;
        });
    ...
    var images = node.append("image")
        ...
        .attr("xlink:href", function (d) {
            if (!d.isHidden && !d.isFixed)
                return "img/" + d.id + ".png";
        })
        .attr("x", "-35px")
        .attr("y", "-35px")
        .attr("width", function (d) {
            if (d.isHidden || d.isFixed) return "0px";
            else return "70px";
        })
